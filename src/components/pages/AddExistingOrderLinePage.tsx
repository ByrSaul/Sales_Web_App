...
const AddExistingOrderLinePage = () => {
  ...
  useEffect(() => {
    setPriceOverride('');
  }, [product?.itemId, variant?.displayProductNumber, order?.customerAccount, order?.currencyCode]);
    ...
    () =>
      order
        ? {
            ...
            orderResponsiblePersonnelNumber: context.user?.personnelNumber ?? '',
            customer: null,
  ...
  const submit = async () => {
    if (!shellDraft || !generatedLines.length || errors.length || mutations.addLine.isPending)
      return;
    setMessage(null);
    const [mainLine, bonusLine] = generatedLines;
    const main = mapNormalLineRequest(shellDraft, mainLine, salesOrderNumber);
    const bonus = bonusLine
      ? mapNormalLineRequest(shellDraft, bonusLine, salesOrderNumber)
      : undefined;
    try {
      ...
      if (error instanceof BonusLineFailure) {
        setMessage(
          `La línea principal se creó. La bonificación ${isAmbiguousError(error.cause) ? 'no pudo verificarse: el resultado es ambiguo.' : `fue rechazada: ${mutationErrorMessage(error.cause)}`} Revise el detalle antes de reintentar.`,
        );
      } else {
        setMessage(
          isAmbiguousError(error)
            ? 'No fue posible determinar si Dynamics creó la línea.'
            : `Dynamics rechazó la línea: ${mutationErrorMessage(error)}`,
        );
      }
    }
  };
    ...
    <div className="space-y-4 pb-12">
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <Card className="p-4 space-y-3">
          ...
          <Button
            fullWidth
            loading={mutations.addLine.isPending}
            disabled={!generatedLines.length || errors.length > 0 || price.isLoading}
            onClick={submit}
          >
            Agregar línea
          </Button>
