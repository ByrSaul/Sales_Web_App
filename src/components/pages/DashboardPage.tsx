...
const ACTIVITY = [
  { icon: 'shopping_cart', iconBg: 'bg-primary/10 text-primary', title: 'Pedido #4529 - Agromás S.A.',    sub: 'Creado hace 2 horas • $12,450.00' },
  { icon: 'person_add',    iconBg: 'bg-blue-50 text-blue-600',   title: 'Nuevo Cliente: Finca Santa Rosa', sub: 'Registrado hoy a las 09:15 AM' },
  { icon: 'receipt',       iconBg: 'bg-red-50 text-error',       title: 'Factura generada: F001-923',      sub: 'Pendiente de envío a cliente' },
];
const QUICK = [
...
const DashboardPage: React.FC = () => (
  <div className="space-y-5">
    ...
    <Card className="relative overflow-hidden border-0 p-8 bg-gradient-to-r from-primary to-primary-container">
      ...
      <span className="material-symbols-outlined absolute -right-4 -top-4 text-[140px] text-white/10 select-none"
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
      ...
      <div className="lg:col-span-8 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          {[
            { label: 'PEDIDOS HOY',         value: '12',  trend: '+8% vs ayer',  color: 'text-primary',   icon: 'shopping_basket', up: true },
            { label: 'PEDIDOS FINALIZADOS',  value: '134', trend: 'Mes en curso', color: 'text-secondary', icon: 'check_circle',    up: true },
          ].map(s => (
            <Card key={s.label} className="p-4 md:p-5">
              <div className="flex items-start justify-between">
                <p className="text-xs text-on-surface-variant uppercase tracking-widest font-bold">{s.label}</p>
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${s.up ? 'bg-primary/10' : 'bg-secondary/10'}`}>
                  <Icon name={s.icon} size={18} className={s.color} />
                </div>
              </div>
              <p className={`text-4xl font-black ${s.color} mt-2 leading-none`}>{s.value}</p>
              <p className={`text-xs ${s.up ? 'text-primary' : 'text-secondary'} flex items-center gap-1 mt-2`}>
                <Icon name="trending_up" size={13} />{s.trend}
              </p>
            </Card>
          ))}
        </div>
        ...
        <Card className="p-4 md:p-5">
          ...
          <div className="divide-y divide-outline-variant/30">
            {ACTIVITY.map((item, i) => (
              <button key={i} className="w-full flex items-center gap-3 py-3 hover:bg-surface-container/60 transition-colors rounded-lg px-2 text-left group">
                ...
                <Icon name="chevron_right" size={16} className="text-on-surface-variant group-hover:text-primary transition-colors flex-shrink-0" />
              </button>
            ))}
          </div>
        </Card>
      ...
      <div className="lg:col-span-4 space-y-4">
        <Card className="overflow-hidden">
          <div className="h-32 bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center relative">
            ...
            <div className="absolute top-2 left-2 flex gap-1">
              <span className="px-2 py-0.5 bg-secondary text-white rounded text-xs font-bold">CLIENTE</span>
        ...
        <div className="grid grid-cols-2 gap-3">
          {QUICK.map(q => (
            <button key={q.label} className="flex flex-col items-center gap-2 p-4 bg-white border border-outline-variant rounded-xl hover:border-primary hover:bg-primary/5 transition-all group">
              <Icon name={q.icon} size={24} className="text-on-surface-variant group-hover:text-primary transition-colors" />
              <span className="text-xs text-on-surface-variant group-hover:text-primary font-medium text-center">{q.label}</span>
            </button>
          ))}
        </div>
      </div>
