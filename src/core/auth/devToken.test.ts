...
const syntheticJwt = (claims: Record<string, string>) =>
  `x.${btoa(JSON.stringify(claims)).replace(/=/g, '')}.x`;
...
describe('temporary development token provider', () => {
  ...
  it('fails closed without a token and never includes token material in errors', () => {
    ...
    try {
      devAccountId(material);
    } catch (cause) {
      expect(String(cause)).not.toContain(material);
    }
  });
  it('tags a missing token as configuration and a missing oid/sub as identity', () => {
    try {
      ...
      expect((cause as DevTokenConfigurationError).kind).toBe('configuration');
    }
    const material = syntheticJwt({ role: 'missing-identity' });
    try {
      devAccountId(material);
      ...
      expect((cause as DevTokenConfigurationError).kind).toBe('identity');
    }
  });
});
