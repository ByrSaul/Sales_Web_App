...
export const useAttachmentUploader = (salesOrderNumber: string) => {
  ...
  return {
    upload: (file: File, description: string) =>
      attachmentService(api).upload(companyId, salesOrderNumber, file, description),
    refresh: () =>
      client.invalidateQueries({ queryKey: attachmentKeys.order(companyId, salesOrderNumber) }),
  };
};
