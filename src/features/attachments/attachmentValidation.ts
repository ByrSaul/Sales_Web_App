...
export const validateAttachment = (file: File, description: string): string | null => {
  if (!allowed.has(extensionOf(file.name))) return 'Solo se permiten archivos PDF, JPG, JPEG o PNG.';
...
export const isPaymentDescription = (description: string) => description.trim().toLowerCase() === 'pago';
