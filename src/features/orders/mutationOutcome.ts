...
export const isAmbiguousError = (error: unknown): boolean =>
  error instanceof ApiError
    ? error.kind === 'timeout' || error.kind === 'network' || error.kind === 'invalid-response' || (error.status ?? 0) >= 500
    : true;

export const mutationErrorMessage = (error: unknown): string =>
  error instanceof ApiError
    ? error.kind === 'timeout' ? 'La operación agotó el tiempo de espera; su resultado es ambiguo.'
    : error.kind === 'network' ? 'Se perdió la conexión; es necesario verificar Dynamics.'
    : error.status ? `Backend rechazó la operación (${error.status}).`
