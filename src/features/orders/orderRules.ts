...
export const canEditOrderLine = (order: ExistingOrder, line: ExistingOrderLine) =>
  ...
  !line.status.toLowerCase().includes('facturado') &&
  !line.status.toLowerCase().includes('invoiced') &&
  Boolean(line.inventoryLotId);
export const canCancelOrderLine = canEditOrderLine;
export const canConfirmOrder = (order: ExistingOrder, lines: ExistingOrderLine[]) =>
